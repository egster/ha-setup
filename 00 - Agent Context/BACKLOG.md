...
4. ⚠️ **Kiosk Mode toggle** — toggle icon visible at sidebar bottom, `input_boolean.fusion_kiosk` toggles correctly. BUT kiosk-mode plugin (v13.0.0) doesn't respond to entity_settings — HA header/sidebar stay visible even with boolean `on` + page reload. Tried: `entity: {id: state}` dict form, explicit `hide_header/hide_sidebar/hide_overflow`, `admin_settings` mirror. Plugin loads per console, just doesn't activate conditionally. Workaround: URL param `?kiosk=true` activates for that session. Needs deeper dive into kiosk-mode v13 syntax or fallback to URL-based trigger.
5. ⚠️ **Sidebar vertical centering** — icons visually start at top of sidebar cell rather than centered. Wrapped sidebar in `custom:mod-card` with `flex-direction: column; justify-content: center; height: 100%` on ha-card — mod-card is visible but centering not taking effect. Might need deeper selector or a different flex target. Non-blocking, cosmetic.
