...
1. 🟡 **Bubble Card popups for room taps — IN PROGRESS** (Phase 7 shipped the YAML scaffolding; Edgar will add the actual popups via HA UI manually). The four popup files (`config/dashboards/fusion/popups/{living-room,kitchen,office,outdoor}.yaml`) are kept as reference YAML — they document the entity manifests, section structures, ApexCharts configs, and FUSION dark-theme styling that the UI-managed popups should mirror. The home-panel room-header `tap_action: navigate, navigation_path: '#popup-<room>'` wires (WP6) are in place + the bottom-nav-hide CSS rule (WP6) is ready for whatever popup mount path Edgar chooses. Why YAML-mode didn't work end-to-end (2026-04-29): mounting Bubble Card popups as siblings of dashboard content in a `vertical-stack` inside a panel-mode `custom:mod-card` causes Bubble Card's popup-wrapper to adopt the dashboard's vertical-stack siblings as the popup's children, so opening any hash renders the whole FUSION shell inside the popup overlay. See DECISIONS 2026-04-29 for full root-cause + alternatives considered.
