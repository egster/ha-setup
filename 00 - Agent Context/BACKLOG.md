...
10. 🟡 **Mobile (phone) responsiveness** — iPad renders cleanly, phone does not. Likely culprits: (a) outer layout-card `margin: 0 16px 0 -84px` + sidebar's 72px column don't leave enough width on a ~375px viewport (content column collapses or horizontal-scrolls); (b) status bar's 8-column grid + 86px left padding pushes half the cells off-screen; (c) room grid `repeat(3, 1fr)` in MAIN FLOOR forces 3 cards into a narrow viewport. **Options**: add CSS `@media (max-width: 600px)` breakpoints via a top-level `card_mod` that collapses the sidebar, stacks floor grids to 1-column, and shrinks statusbar to essentials; OR route mobile users to BubbleDash v4 (once archived, keep it live for mobile only); OR create a separate `dashboard-fusion-mobile` variant. Investigate which panels are actually used on phone before deciding — if it's mostly glancing at status + lights, a trimmed mobile variant is lighter than retrofitting FUSION. Added 2026-04-24.
