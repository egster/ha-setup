...
- [~] WP5a — popup template + Living Room popup — committed on `phase7/wp5a-template-livingroom`, Gate 2 reviewed (APPROVED after one round of fixes for missing shell.yaml include + Automations-header inline-vs-template), Gate 3 deploy pending. 17 tests added (TEST-400 … TEST-416), 8 yaml_schema currently pass, 14 browser-deferred + 1 entity test will run post-deploy. Merged onto WP4's new shell by wrapping state-switch in a vertical-stack so the popup sits as a sibling of the state-switch (always in DOM, viewport-agnostic).
- [ ] WP5b — Kitchen popup
- [ ] WP5c — Office popup
- [ ] WP5d — Outdoor popup
