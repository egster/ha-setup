...
- [ ] WP5d — Outdoor popup — **NOT IMPLEMENTED.** Branch `phase7/wp5d-outdoor` is at the same SHA as `main` post-WP5a (zero commits beyond main). Only test definitions (TEST-450 … TEST-460) survive in `stash@{0}` (`WP5d-outdoor-WIP-preserved-by-WP5b-session-2026-04-28`). The actual `config/dashboards/fusion/popups/outdoor.yaml` was never written. WP5d session needs to be re-launched per the brief (`00 - Agent Context/fusion-phase7/WP5d-outdoor.md`) before WP6 can begin — the WP6 prerequisite line above (WP5d ✅) is unmet.
